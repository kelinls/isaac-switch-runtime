...
EVIDENCE = (
    ROOT
    / "analysis/stage26-game-safe-point"
...
RESERVED_INTERVALS = (
    ...
    range(0x68CC00, 0x68CC20),
    range(0x68CC20, 0x68CC40),
)


class Stage26GameSafePointTests(unittest.TestCase):
    def test_fixed_evidence_locks_game_update_safe_point_and_independent_relay_cave(self):
        evidence = json.loads(EVIDENCE.read_text(encoding="utf-8"))

        self.assertEqual(evidence["schema_version"], "stage26-game-safe-point-v1")
        ...
        self.assertEqual(evidence["safe_point"]["bl_mnemonic"], "BL")
        self.assertEqual(evidence["safe_point"]["caller_saved_live_after_call"], [])
    ...
    def test_exporter_stays_read_only_and_checks_fixed_callsite(self):
        script = ROOT / "analysis/ghidra/scripts/ExportStage26GameSafePoint.java"
        self.assertTrue(script.exists(), "Stage 26 safe-point exporter must exist")
        source = script.read_text(encoding="utf-8")
        for required in (
            "003f9058",
            "003f905c",
            "0067c9e0",
            "caller_saved_live_after_call",
