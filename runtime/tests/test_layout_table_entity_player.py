"""`tools/layout_tables/entity_player.json`（`Entity_Player`）的门禁：证据要能重新证明、生成物不许漂移、命名口径不许放宽。
...
TABLE = ROOT / "tools" / "layout_tables" / "entity_player.json"
HEADER = ROOT / "runtime" / "source" / "entity_player_layout.hpp"
NRO = ROOT / (
...
class LayoutTableTests(unittest.TestCase):
    ...
    def test_confirmed_rows_need_name_justifying_evidence(self):
        ...
        本表里六个属性字段各自都有取值锚点（`Entity_Player::Init` 写进去的默认值、麻痹药丸分支的 240/120 帧、
        读 `CanFly` 之后写 `Entity+0x1E0` 的层号），所以六行都是 confirmed。
        """
        for row in self.table["rows"]:
