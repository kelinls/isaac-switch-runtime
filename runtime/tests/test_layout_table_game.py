"""`tools/layout_tables/game.json`（`Game`）的门禁：证据要能重新证明、生成物不许漂移、命名口径不许放宽。
...
TABLE = ROOT / "tools" / "layout_tables" / "game.json"
HEADER = ROOT / "runtime" / "source" / "game_layout.hpp"
NRO = ROOT / (
...
class LayoutTableTests(unittest.TestCase):
    ...
    def test_confirmed_rows_need_name_justifying_evidence(self):
        ...
        本表里 `ShakeTimer`/`ScreenShakeOffset` 两行只有『引擎符号名 + 行为』支撑（拿不出取值常量）⇒ 标 candidate；
        两个 GOT 槽行同理（命名来自 NRO 重定位表，但页基址那一半工具核不到）。
        """
        for row in self.table["rows"]:
