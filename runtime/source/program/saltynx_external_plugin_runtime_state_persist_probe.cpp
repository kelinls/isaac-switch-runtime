...
using RuntimeStatePersistProbe = std::uint64_t (*)();
...
constexpr char kRuntimeStatePersistProbeSymbol[] =
...
std::uint32_t RunSaltyNxExternalPluginProbe() {
    ...
    const auto runtime_probe =
        reinterpret_cast<RuntimeStatePersistProbe>(g_find_symbol(kRuntimeStatePersistProbeSymbol));
    if (runtime_probe == nullptr) {
        return registration;
    }
    return registration | (static_cast<std::uint32_t>(runtime_probe()) << 8);
}
