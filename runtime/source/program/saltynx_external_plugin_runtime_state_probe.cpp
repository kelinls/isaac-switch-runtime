...
using RuntimeStateProbe = std::uint64_t (*)();
...
constexpr char kRuntimeStateProbeSymbol[] = "IsaacModRuntime_SaltyStateContainerProbe";
...
std::uint32_t RunSaltyNxExternalPluginProbe() {
    ...
    const auto runtime_state_probe =
        reinterpret_cast<RuntimeStateProbe>(g_find_symbol(kRuntimeStateProbeSymbol));
    if (runtime_state_probe == nullptr) {
        return registration;
    }
    return registration | (static_cast<std::uint32_t>(runtime_state_probe()) << 8);
}
