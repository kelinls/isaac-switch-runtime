...
// 端口类型 → 安装器要的模块描述。与 `got_slot_hook_adapter.cpp` / `exlaunch_hook_adapter.cpp`
// 里那份换算是同一套口径（`HookTarget` 与 `TargetModule` 字段一一对应）。
...
Status RelaySlotHookAdapter::Install(HookId id, const HookTarget& target) noexcept {
    ...
    // 本后端只实现 `GameStart` 那一处；别的挂点由路由表交给各自的适配器。
    // 走到这里的其它 `HookId` 是配置错误，**响亮失败**而不是静默成功。
    if (id != HookId::GameStart) {
        return Status{StatusCode::Rejected};
    }
    if (TryInstallGameStartRelay(ModuleOf(target)) != GameStartRelayInstallResult::Success) {
        return Status{StatusCode::Rejected};
    }
...
bool RelaySlotHookAdapter::IsInstalled(HookId id) const noexcept {
    const std::size_t index = static_cast<std::size_t>(id);
